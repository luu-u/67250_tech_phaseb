...
/*  Interactive Slideshow Gallery (jQuery)  */
...
$(document).ready(function () {
    ...
    // Show a specific slide with a fade transition
    function goToSlide(index) {
        var nextIndex = (index + slides.length) % slides.length;
        if (nextIndex === currentSlide) return;
        slides.eq(currentSlide).removeClass("active").stop(true).fadeOut(400);
        dots.eq(currentSlide).removeClass("active");
        slides.eq(nextIndex).stop(true).fadeIn(400, function () {
